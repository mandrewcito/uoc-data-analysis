...
for (i in 1:(ncol(titanicData) - 1)) {
  ...
  rownames(corr_matrix)[nrow(corr_matrix)] <- colnames(titanicData)[i]
  }
} 
print(corr_matrix)
```
Al no estar en valores cercanos a uno, tanto negativo como positivo. No hay ninguna variable numérica correlacionada con la supervivencia. La más fuerte sería la del precio del billete, pero no es suficiente.
